...
if uploaded_files:
    ...
    if raw_list:
        ...
        with st.spinner('执行红线并集概率审计...'):
            # 基础局计算
            audit_res = main_df.apply(lambda r: pd.Series(audit_engine(r, cm, base_score, burst_win, burst_thr)),
                                      axis=1)
        ...
        for h_v, gp_h in df_fact.groupby('初始手牌'):
            ...
            row = {"手牌数": h_v, "牌集总数": total_unique_jid, "✅ 通过(去重)": total_pass_jid,
        ...
        f_h = st.multiselect("手牌维度", sorted(df_fact['初始手牌'].unique()),
                             default=sorted(df_fact['初始手牌'].unique()))
        f_s = st.radio("判定过滤", ["全部", "通过", "拒绝"], horizontal=True)

        view_df = df_fact[df_fact['初始手牌'].isin(f_h)].copy()
        if f_s == "通过":
            view_df = view_df[view_df['is_pass'] == 1]
        elif f_s == "拒绝":
            view_df = view_df[view_df['is_pass'] == 0]
